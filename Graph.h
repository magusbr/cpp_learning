...
class Graph
{
    public:
        ...
        Graph(const unsigned int& num_nodes, const double& edge_density, const unsigned double& min_distance, const unsigned double& max_distance);
        ...
        bool add_edge(const unsigned int& x, const unsigned int& y, const unsigned double& distance); // adds to graph the edge from x to y, if it is not there.
        bool rem_edge(const unsigned int& x, const unsigned int& y); // removes the edge from x to y, if it is there.
        unsigned double get_edge_value(const unsigned int& x, const unsigned int& y); // returns the value associated to the edge (x,y).
        bool set_edge_value(const unsigned int& x, const unsigned int& y, const unsigned double& distance); // sets the value associated to the edge (x,y) to distance.
    ...
    private:
        ...
        vector<vector<unsigned double>> dist_matrix;

        double prob(); // double random from 0.0 to 1.0
        int prob_range(const unsigned double& min_distance, const unsigned double& max_distance); // int range from min_distance to max_distance
        void randomize(const double& edge_density, const unsigned double& min_distance, const unsigned double& max_distance); // generate graph with size num_nodes with random edges with size [min_distance, max_distance]
};
